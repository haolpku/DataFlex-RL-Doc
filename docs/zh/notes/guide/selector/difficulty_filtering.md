...
例如 `T_low = 0.2, T_high = 0.8`。此带为**开区间**(严格不等式,与 `ThresholdBandSelector`
的 `keep = (s > low) & (s < high)` 一致),故 pass-rate *恰好*等于 `T_low` 或 `T_high` 的组会被丢弃。
这**推广了 DAPO dynamic sampling** —— 后者是退化带 `(0, 1)`,只排除恰好 0 和恰好 1。
